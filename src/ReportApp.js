    ...
    return <div>
        ...
        <Section>
            <div className={"flex flex-col justify-center items-center mb-48"}>
                <h1 className={"font-bold mb-3 text-4xl"}>Welcome to Mediport.</h1>
                <span
                    className={"text-xl"}>This report is an overview of your last appointment with Dr. Baumgartner.</span>
            </div>
                    ...
                    <div className={"flex flex-wrap w-full"}>
                        {report.issues_global.map(issueKey => {
                            ...
                            return <p key={`iss-${issueKey}`}
                                ...
                                <span data-tip={`Chat about this topic`} onClick={() => startChat(diag.display_name)} className={"chaticon opacity-50 cursor-pointer hover:opacity-100 transition duration-200 ease float-right w-8 absolute right-4 bottom-2"}>
                                    <img src={ChatIcon}/>
